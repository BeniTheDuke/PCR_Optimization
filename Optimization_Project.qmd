...
Mathematical Formulation of the PCR Primer Design Optimization Problem

1.1 Optimization Cost function
...
$C_T$: penalizes melting temperatures far from the target and penalizes mismatch between the two primers' melting temperatures.

$C_{GC}$: penalizes deviation of each primer's GC fraction from the target.

$C_R$: penalizes long homopolymer runs (repeats).

$C_H$: penalizes hairpin-forming potential using each sequence's hairpin score.

$C_D$: penalizes primer-dimer risk from complementary runs between the two primers (especially near sequence ends).
...
Calculate GC content of a sequence $s$

$T_m(s)$

Calculate melting temperature of a sequence $s$

$R(s)$

Get the longest repeat in a sequence $s$

$r_0$

Minimum length of a repeat to affect the cost (3)

$H(s)$

Calculate the hairpin score of sequence $s$

$D(s_1, s_2)$

Calculate the dimer score of sequence $s_1$ and $s_2$
...
2.2 Genetic Algorithm Optimizer
...
The source code can be found in our git repository: https://github.com/BeniTheDuke/PCR_Optimization. The implementation with all the required classes are in th src folder. The visualizations for the comparison were generated with /optimizer_comparison.qmd.