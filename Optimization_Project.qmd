...
The Genetic Algorithm (GA) is a search technique that mimcs natural selection to find optimal solutions by iteratively refining a population of candidate solutions (McKee, 2024). Each individual represents a primer pair $p_1, p_2$ of fixed length $L=25$ over $\{A, C, G, T\}$, and is evaluated using the defined cost function. At each generation, parents are selected via tournament selection, single-point crossover exchanges nucleotide segments between paired parents, and mutation alters each nucleotide with probability $1/L$ following Ochoa (2002). Elitism preserves the best solution found so far by replacing the worst individual in each new generation. The final output is the primer pair with the minimum cost
...
In terms of computational efficiency, Figure 2 (Runtime histogram) shows that SA is consistently the fastest algorithm, while ACO is the most computationally expensive, with GA and Random Search in between. Similarly.
